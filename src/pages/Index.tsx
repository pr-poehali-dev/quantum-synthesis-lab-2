...
const MESSAGES = [
  ...
  },
  {
    ...
    avatar: "М",
    color: "#ed4245",
    time: "Сегодня в 10:43",
    text: "Привет! Отлично, только что запустил новый проект 🚀",
    isBot: false,
  },
  {
    id: 3,
    ...
    text: "Ребята, кто смотрел последний стрим? Там было что-то невероятное!",
    isBot: false,
  },
  {
    id: 4,
    ...
    avatar: "А",
    color: "#5865f2",
    time: "Сегодня в 10:47",
    text: "Да, я смотрела! Вообще огонь 🔥 Кстати, скоро ивент в #новости",
    isBot: false,
  },
  {
    id: 5,
    ...
    avatar: "М",
    color: "#ed4245",
    time: "Сегодня в 10:50",
    text: "Буду! Кто ещё идёт?",
    isBot: false,
  },
];
    ...
    <div className="flex h-screen bg-[#f2f3f5] overflow-hidden font-sans">
          ...
          <button
            ...
            className={`
              ...
              ${s.id === activeServer
                ...
                : "bg-white rounded-3xl hover:rounded-2xl hover:bg-[#5865f2]"}
            `}
          >
            {s.id === activeServer && (
      ...
      `}>
        ...
        <div className="flex-1 overflow-y-auto py-2 px-2">
          ...
          <div className="mb-1">
            ...
            {CHANNELS.map((ch) => (
              <button
                key={ch.name}
                onClick={() => { setActiveChannel(ch.name); setSidebarOpen(false); }}
                className={`
                  w-full flex items-center gap-1.5 px-2 py-1.5 rounded text-sm cursor-pointer transition-colors
                  ${activeChannel === ch.name
                    ? "bg-[#d5d8de] text-[#060607] font-medium"
                    : "text-[#4e5058] hover:bg-[#d5d8de] hover:text-[#313338]"}
                `}
              >
        ...
        <div className="p-2 bg-[#d5d8de] flex items-center gap-2">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-[#4e5058] text-xs truncate">В сети</div>
          </div>
          <div className="flex gap-1">
            <button className="w-7 h-7 flex items-center justify-center rounded hover:bg-[#c4c9d4] text-[#4e5058] hover:text-[#313338] transition-colors">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="h-12 bg-white border-b border-[#e3e5e8] flex items-center px-4 gap-3 flex-shrink-0 shadow-sm">
          <button
            className="md:hidden text-[#4e5058] hover:text-[#313338] mr-1"
          ...
          <span className="text-[#4e5058] text-sm hidden sm:block truncate">Добро пожаловать в #{activeChannel}!</span>
          <div className="ml-auto flex items-center gap-3">
            <button className="text-[#4e5058] hover:text-[#313338] transition-colors hidden sm:block">
            ...
            <div className="relative hidden md:block">
              ...
              <input
                className="bg-[#f2f3f5] border border-[#e3e5e8] rounded pl-8 pr-3 py-1 text-sm text-[#313338] placeholder:text-[#4e5058] outline-none focus:border-[#5865f2] w-36"
                placeholder="Поиск"
              />
            </div>
          </div>
          ...
          <div className="flex-1 flex flex-col min-w-0">
            <div className="flex-1 overflow-y-auto px-4 py-4 space-y-1">
              ...
              <div className="mb-6 pb-4 border-b border-[#e3e5e8]">
                ...
                <p className="text-[#4e5058] text-sm">Это начало канала #{activeChannel}. Пишите сюда!</p>
              </div>
              ...
              {messages.map((msg, i) => {
                ...
                return (
                  <div
                    key={msg.id}
                    className={`flex gap-3 group hover:bg-[#e9eaec] rounded px-2 py-0.5 transition-colors ${isGrouped ? "ml-0" : "mt-4"}`}
                  >
                    {!isGrouped ? (
                      <div
                        className="w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 text-white font-semibold text-sm"
                    ...
                    <div className="flex-1 min-w-0">
                      {!isGrouped && (
                        <div className="flex items-baseline gap-2 mb-0.5">
                          <span className="font-semibold text-[#060607] text-sm hover:underline cursor-pointer">
                            {msg.author}
                          </span>
          ...
          <div className="hidden lg:flex w-60 bg-[#f2f3f5] flex-col flex-shrink-0 overflow-y-auto px-3 py-4 border-l border-[#e3e5e8]">
            ...
            <div className="mb-4">
              ...
              {MEMBERS_ONLINE.map((m) => (
                <button
                  key={m.name}
                  className="w-full flex items-center gap-3 px-2 py-1.5 rounded hover:bg-[#e3e5e8] transition-colors"
                >
                  <div className="relative flex-shrink-0">
                    <div
                    ...
                    <div
                      className="absolute -bottom-0.5 -right-0.5 w-3 h-3 border-2 border-[#f2f3f5] rounded-full"
                      style={{ background: STATUS_COLORS[m.status] || "#4e5058" }}
                    />
                  </div>
                  <div className="flex-1 min-w-0 text-left">
                    <div className="text-[#313338] text-sm font-medium truncate">{m.name}</div>
            ...
            <div>
              ...
              {MEMBERS_OFFLINE.map((m) => (
                <button
                  key={m.name}
                  className="w-full flex items-center gap-3 px-2 py-1.5 rounded hover:bg-[#e3e5e8] transition-colors opacity-50"
                >
                  <div className="relative flex-shrink-0">
